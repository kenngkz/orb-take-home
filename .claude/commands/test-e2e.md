...
### Agent 3: "Chat, Q&A, and Citations"
Scenarios: Send message and receive response, Multi-document question, Citations appear in response, Citation click navigates to page.
Instructions: Navigate to http://localhost:5173. Create a conversation, upload both PDFs from `e2e/fixtures/`. Send "What is the annual rent in the lease agreement?" and wait up to 30s for streaming response. Take screenshot — **save all screenshots to `e2e-screenshots/` with descriptive names** (e.g., `e2e-screenshots/chat-01-single-doc-response.png`). Verify the response has: (a) small blue numbered markers inline in the text, (b) citation pills below the response showing filename and page number with matching number badges. Then send "Compare the proposed rent in the rent review with the current rent in the lease" and wait for response. Take screenshot. Verify responses reference correct documents by name. Then click on one of the citation pills below a response and take a screenshot — verify the document viewer navigates to the cited page (check the page indicator at the bottom). Report verdict and notes for each scenario.
